...
| 0xAA 0x55     | CMD_TYPE      | Big-Endian    | Data bytes    | IEEE 802.3    | 0x0D          |
+---------------+---------------+---------------+---------------+---------------+---------------+
```
...
- **CRC32 (`4 Bytes`, Big-Endian):** IEEE 802.3 CRC-32 (Polynomial `0x04C11DB7`) calculated over `[Command + Length + Payload]`, verified natively by the STM32 hardware CRC engine.
...
| `0x02` | `CMD_ERASE` | ESP32-S3 $\rightarrow$ STM32 | Erase application flash region (`0x08004400`..`0x08010000`) |
| `0x82` | `RESP_ERASE` | STM32 $\rightarrow$ ESP32-S3 | Flash erase status (`0x00`: Success, `0x01`: Error) |
| `0x03` | `CMD_WRITE_DATA` | ESP32-S3 $\rightarrow$ STM32 | Write binary chunk (`Flash Offset [4B]` + `Chunk Data [NB]`) |
...
- **`CMD_START_UPDATE` Payload (8 Bytes):**
  ...
  - Bytes [4..7]: `Total Expected CRC32 (uint32_t Big-Endian)`
